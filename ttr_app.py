...
# =============================================================================
# BLOQUE 2: MOTOR TTR (V14.0 - CORRECCIÓN TO_UPPERCASE)
# =============================================================================

def procesar_dmk_v14(f_zip, df_v, df_tarifas, f_ener, f_corte):
    try:
        ...
        # 2. PREPARACIÓN DEL NOMENCLADOR (Selección Quirúrgica)
        ...
        # 3. PARQUE MÓVIL (Corrección: to_uppercase en lugar de upper)
        pme_pl = pl.from_pandas(pd.read_excel(f_ener)).lazy().select([
            pl.col("DOMINIO").cast(pl.Utf8).str.strip_chars().str.to_uppercase(),
        ...
        # 6. UNIÓN CON ENERGÍAS Y AGRUPACIÓN
...
if 'tar_v14' not in st.session_state: st.session_state.tar_v14 = None
if 'ttr_v14' not in st.session_state: st.session_state.ttr_v14 = None

t_tabs = st.tabs(["💰 1. CONFIGURAR TARIFAS", "⚡ 2. LIQUIDACIÓN DMK"])

with t_tabs[0]:
    f_n = st.file_uploader("Cuadro Noviembre", key="up_n_14")
    if f_n:
        c = st.columns(5)
        ...
        if st.button("📊 Proyectar Tarifas"):
            st.session_state.tar_v14 = motor_tarifas_proyeccion(pd.read_excel(f_n), m)
    if st.session_state.tar_v14 is not None: st.dataframe(st.session_state.tar_v14)

with t_tabs[1]:
    if st.session_state.tar_v14 is not None:
        ca, cb = st.columns(2)
        fv = ca.file_uploader("Nomenclador V", key="up_v_14")
        fe = cb.file_uploader("Energías", key="up_e_14")
        fzip = st.file_uploader("DMK (ZIP/CSV)", key="up_z_14")
        dt_corte = st.date_input("Fecha Cambio de Tarifa:", datetime(2026, 2, 14))
        
        if fv and fe and fzip and st.button("🚀 GENERAR TTR FINAL"):
            with st.spinner("Procesando (Sintaxis Polars corregida)..."):
                st.session_state.ttr_v14 = procesar_dmk_v14(fzip, pd.read_excel(fv), st.session_state.tar_v14, fe, str(dt_corte))
            
            if st.session_state.ttr_v14 is not None:
                st.success("TTR Finalizado.")
                st.download_button("📥 DESCARGAR EXCEL", preparar_descarga(st.session_state.ttr_v14), "Liquidacion_TTR_Febrero.xlsx")
                st.dataframe(st.session_state.ttr_v14.head(10))
    else:
